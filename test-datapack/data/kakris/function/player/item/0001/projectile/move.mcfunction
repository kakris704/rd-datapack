# Projectile Tick
    ...
    ## hantei
        ...
        execute as @e[dx=0,dy=0,dz=0,type=!#kakris:non_target] positioned ~-1 ~-1 ~-1 if entity @s[dx=0,dy=0,dz=0] if function kakris:utils/projectile/uuid_check unless entity @s[tag=RD.0001.DartsProjectile] run function kakris:player/item/0001/projectile/hit with entity @e[tag=RD.0001.DartsProjectile,sort=nearest,limit=1] data
